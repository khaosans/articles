...
def test_enhanced_diagrams():
    ...
    for mmd_file in enhanced_files:
        ...
        assert 'classDef' in content, f"Enhanced diagram {mmd_file.name} missing classDef"
        assert 'font-size' in content, f"Enhanced diagram {mmd_file.name} missing font-size"
        assert 'font-weight' in content, f"Enhanced diagram {mmd_file.name} missing font-weight"
...
def test_no_duplicate_enhanced():
    ...
    assert len(duplicate_files) == 0, f"Found duplicate enhanced files: {[f.name for f in duplicate_files]}"
...
def test_ci_workflow_includes_mcp():
    ...
    assert 'Test Mermaid diagrams with MCP' in content, "MCP testing step not found in CI"
