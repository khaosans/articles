...
def test_diagram_with_mcp(diagram_path):
    ...
    try:
        ...
        if ":::" in diagram_content:
            ...
            class_defs = [
                line.split()[1] for line in diagram_lines if line.startswith("classDef")
            ]
        ...
        for char in problematic_chars:
            if char in diagram_content and not any(
                char in line
                for line in diagram_lines
                if "style" in line or "classDef" in line
            ):
...
def test_all_diagrams():
    ...
    for i, mmd_file in enumerate(sorted(mmd_files), 1):
        ...
        if is_valid:
            ...
            results["broken"] += 1
            results["broken_files"].append({"file": mmd_file.name, "error": message})
            print(f"❌ {message}")
    ...
    if results["broken_files"]:
        ...
        print("   3. Check for incomplete arrow syntax")
        print("   4. Review special characters in node labels")
        print("   5. Test with MCP server for rendering issues")
...
"""
    ...
    print("🔧 Auto-fix script generated: fix_broken_diagrams.py")
...
if __name__ == "__main__":
    ...
    if results["broken"] > 0:
        ...
        print("💡 Run 'python3 fix_broken_diagrams.py' to attempt automatic fixes")
    else:
