    ...
    try:
        ...
        for pattern in problematic_patterns:
            if re.search(pattern, diagram_content):
                return False, f"Problematic pattern detected"
        ...
        if 'graph' in first_line or 'flowchart' in first_line:
            ...
            for line in lines:
                line = line.strip()
                if '[' in line and ']' in line and not line.startswith('style') and not line.startswith('classDef'):
                    has_nodes = True
                    break
...
def test_single_diagram(args):
    ...
    try:
        ...
        success, message = test_diagram_with_mcp_simulation(diagram_content, mmd_file.name)
...
def test_all_diagrams_parallel():
    ...
    args_list = [(mmd_file, i+1, total_files) for i, mmd_file in enumerate(sorted(mmd_files))]
    
    # Use ThreadPoolExecutor for parallel processing
    with ThreadPoolExecutor(max_workers=min(8, total_files)) as executor:
        # Submit all tasks
        future_to_file = {executor.submit(test_single_diagram, args): args[0] for args in args_list}
...
def generate_quick_report(results):
    ...
    if results['errors']:
        ...
        print(f"\n🔧 Quick fixes needed:")
        print(f"   1. Fix syntax errors (brackets, parentheses, braces)")
        print(f"   2. Define missing class references")
        print(f"   3. Fix incomplete arrows")
        print(f"   4. Escape special characters")
...
"""
    ...
    print(f"🔧 Quick fix script generated: quick_fix_diagrams.py")
...
if __name__ == "__main__":
    ...
    if results and results['failed'] > 0:
        ...
        print(f"💡 Run 'python3 quick_fix_diagrams.py' for automatic fixes")
    elif results:
