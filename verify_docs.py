...
def validate_mermaid_syntax(mermaid_code):
    ...
    for i, line in enumerate(lines, 1):
        ...
        if (
            ...
            or line.startswith("gantt")
            or line.startswith("pie")
            or line.startswith("radar")
            or line.startswith("stateDiagram")
            or line.startswith("classDiagram")
            or line.startswith("mindmap")
        ):
            continue
...
def verify_file(file_path):
    ...
    if mermaid_blocks:
        ...
        for i, mermaid_code in enumerate(mermaid_blocks, 1):
            ...
            if mermaid_issues:
                issues.extend(
                    [f"Mermaid diagram {i}: {issue}" for issue in mermaid_issues]
                )
